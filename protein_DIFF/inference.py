...
def check_seq_fix_pos(seq, fix_pos):
    for pos, aa in fix_pos:
        if seq[pos] != aa:
            return False
    return True
...
if __name__ == '__main__':
    ...
    input_feat_dim = target_protein_test_dataset[0].x.shape[1]+target_protein_test_dataset[0].extra_x.shape[1]
    ...
    trainer.load(milestone=9,filename=args.ckpt)
    ...
    if args.fix_pos_file is not None:
        ...
        for line in lines:
            pos = int(line[1:]) - 1
            aa = line[0]
            assert original_seq[pos] == aa, f"original seq {original_seq[pos]} != {aa}, pos {pos}"
    ...
    for i in range(args.gen_num):
        with torch.no_grad():
            while True:
                ...
                #     if aa == "C":
                #         print(prob[idx,:])
                if check_seq_fix_pos(seq, [(pos, original_seq[pos]) for pos in cond_index]):
                    seq_list = list(seq)
                    for idx, aa in enumerate(seq):
                        if idx in cond_index:
                            continue
                        # if aa == "C":
                        #     seq_list[idx] = original_seq_list[idx]
                    seq = "".join(seq_list)
                    break
