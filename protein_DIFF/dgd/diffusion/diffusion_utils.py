...
def sample_discrete_features(probX, probE=False, node_mask=False):
    ...
    if probE:
        ...
        return PlaceHolder(X=X_t, E=torch.zeros(X_t.shape[0], 0).type_as(X_t), y=torch.zeros(X_t.shape[0], 0).type_as(X_t))

def compute_posterior_distribution(M, M_t, Qt_M, Qsb_M, Qtb_M):
    ''' M: X or E
        Compute xt @ Qt.T * x0 @ Qsb / x0 @ Qtb @ xt.T
        M is the data, e.g. X or pred_X
        M_t is the probibilty matrix 20*20
        Qt_M is the transit matrix for one step
        Qsb_M is the cumprod transit matrix for time = s (Qs_bar)
        Qtb_M is the cumprod transit matrix for time = t (Qt_bar)
    '''
    ...
    product = left_term * right_term    # (bs, N, d) eq1 in DiGRESS paper
...
def mask_distributions(true_X,pred_X,  node_mask,true_E=False, pred_E= False):
