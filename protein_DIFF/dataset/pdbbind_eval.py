...
import pandas as pd
import sys
from utils import normalize, safe_index, one_hot_res, log, dihedral, NormalizeProtein, dataset_argument

warnings.filterwarnings("ignore")

class PdbbindEvaluate(Dataset):
    r"""
    Args:
    ...
    def __init__(self, root: str, name: str, raw_dir: str,
        ...
        # self.set_length = os.listdir(self.r)
        ...
        self.length_protein = len(self.processed_file_names)

        if self.random_sampling:
            self.length_total = self.length_protein -1
        else:
            self.length_total = self.nums_amino_cum[-1]
    ...
    def mk_saved_graph_path(self) -> str:
        dir_name = os.path.join(self.root, self.name.capitalize(), self.graph_name)
    ...
    def saved_protein_name_txt(self) -> str:
        protein_names = os.path.join(self.root, self.name.capitalize(), 'protein_name.txt')
        return protein_names
    
    @property
    def saved_protein_name(self) -> str:
        protein_names = os.path.join(self.root, self.name.capitalize(), 'protein_name.pt')
        return protein_names
    
    @property
    def saved_amino_cum(self) -> str:
        amino_cum_name = os.path.join(self.root, self.name.capitalize(), 'amino_cum.pt')
        return amino_cum_name

    @property
    def processed_dir(self) -> str:
        return os.path.join(self.root, self.name.capitalize(), self.processed_name)
    ...
    def process(self):
        ...
        for idx in tqdm(range(protein_length)):
            saved_prcessed_name = os.path.join(self.processed_dir, f'data_{filenames[idx]}')
            ...
            new_data = Data.clone(data)
            new_data.x = data.x[:,:20]
            new_data.extra_x = torch.cat([data.x[:,20:],data.mu_r_norm],dim = 1)

            torch.save(new_data, saved_prcessed_name)
            self.nums_amino_cum = torch.cat((self.nums_amino_cum.reshape(-1), torch.tensor(data.x.shape[0]).reshape(-1)))
    ...
    def generate_protein_graph_evaluation(self):
        ...
        for idx, name in enumerate(tqdm(names)):        
            ...
            for filename in os.listdir(protein_dir):
                ...
                if (suffix == '.pdb') and (filename != 'unrelaxed_model_1.pdb'):
                    protein_filename = os.path.join(protein_dir,filename)
                    break
            ...
            rec_graph = self.get_calpha_graph(rec, c_alpha_coords, n_coords, c_coords)
            if not rec_graph:
                self.wrong_proteins.append(name)
                continue
            score_info = self.generate_score_matrix(protein_dir, protein_length = len(c_alpha_coords))
            if score_info:
                rec_graph.score_matrix, rec_graph.mutat_mask = score_info[0], score_info[1]
            else:
                continue
            rec_graph.protein_idx = idx
            torch.save(rec_graph,os.path.join(self.saved_graph_path, name + '.pt'))

    def generate_score_matrix(self, protein_dir, protein_length):
        file = ''
        protein_name = protein_dir.split('/')[-1]
        for filename in os.listdir(protein_dir):
            if filename == protein_name + '.1.tsv':
                file = os.path.join(protein_dir,filename)
                break
        if file == '':
            return None
        df = pd.read_csv(file, sep = '\t')
        # if filename.find('F7YBW8') > -1:
        #     df['score'] = -df['score']
        deleted_id = []
        type1, type2, location = [], [], []
        for i in df.index:
            if '_' in df.loc[i,'mutant']:
                # e.g. R663_
                deleted_id.append(i)
                continue
            type1.append(self.amino_acids_type.index(df.loc[i,'mutant'][0]))
            location.append(int(df.loc[i,'mutant'][1:-1]))
            type2.append(self.amino_acids_type.index(df.loc[i,'mutant'][-1]))
        df = df.drop(index = deleted_id)
        df['orig_type'] = type1
        df['change_type'] = type2
        df['location'] = location
        score_matrix = torch.zeros((protein_length,20),dtype=float)
        mutat_mask = score_matrix.ge(1)
        for i in df.index:
            score_matrix[df.loc[i,'location']-1,df.loc[i,'change_type']] = df.loc[i,'score']
            mutat_mask[df.loc[i,'location']-1,df.loc[i,'change_type']] = True
        mutat_mask = torch.tensor(mutat_mask)
        # score_matrix = torch.softmax(score_matrix,dim=1)
        return score_matrix, mutat_mask
    ...
    def get_calpha_graph(self, rec, c_alpha_coords, n_coords, c_coords):
    ...
    def remove_node(self, graph, node_idx):
        ...
        # delete node
        new_graph.x = torch.cat([new_graph.x[:node_idx,:], new_graph.x[node_idx+1:,:]])
        new_graph.pos = torch.cat([new_graph.pos[:node_idx,:], new_graph.pos[node_idx+1:,:]])
        new_graph.mu_r_norm = torch.cat([new_graph.mu_r_norm[:node_idx,:], new_graph.mu_r_norm[node_idx+1:,:]])
    ...
    def get_edge_features(self, src_list, dst_list, dist_list, divisor=4):
        ...
        dist_fea = self.distance_featurizer(dist_list, divisor = divisor)  # avg distance = 7. So divisor = (4/7)*7 = 4
    ...
    def get_statistic_info(self):
        ...
        print(f'Min Nodes: {num_node_min:.2f} Max Nodes: {num_node_max:.2f}. Avg Nodes: {num_node_avg:.2f}')
        print(f'Min Edges: {num_edge_min:.2f} Max Edges: {num_edge_max:.2f}. Avg Edges: {num_edge_avg:.2f}')
    ...
    def get(self, idx):
        ...
        if self.random_sampling:
            ...
            select_residual = remaining_residual[torch.multinomial(torch.ones(remaining_residual.shape[0]), 1).item()]
            select_index = (data.x[:,:20].argmax(dim=1)==select_residual).nonzero().view(-1)              
            subidx = select_index[torch.multinomial(torch.ones(select_index.shape[0]), 1).item()]
        else:    
            subidx = int(idx - self.nums_amino_cum[idx_protein])
        generate_graph = self.generate_one(data, subidx,idx_protein)
        if self.use_micro:
            generate_graph = self.get_calpha_graph_single(generate_graph, idx_protein, subidx)
        return generate_graph
   
    # def generate_one(self,graph,amino_idx):
    #     graph2 = Data.clone(graph)
    #     del graph2.distances
    #     del graph2.edge_dist
    #     graph2.x[amino_idx,:] = 0
    #     graph2.mu_r_norm[amino_idx,:] = 0
    #     graph2.y = np.argmax(graph.x[amino_idx,:self.num_residue_type])
    #     return graph2
    def generate_one(self,graph,amino_idx = 0,idx_protein=0):
        # print('one')
        graph2 = Data.clone(graph)
        graph2.x[amino_idx,:] = 0
        graph2.mu_r_norm[amino_idx,:] = 0
        graph2.y = np.argmax(graph.x[amino_idx,:self.num_residue_type])
        graph2.score_matrix = torch.reshape(graph.score_matrix[amino_idx,:],(1,-1))
        graph2.mutat_mask = torch.reshape(graph.mutat_mask[amino_idx,:],(1,-1))
        graph2.protein_idx = idx_protein
        return graph2
...
if __name__ == '__main__':  
    args = dataset_argument(n =7)
    train_dataset = PdbbindEvaluate(args['root'],args['name'], args['raw_dir'],
                    divide_num= args['divide_num'], divide_idx=args['divide_idx'],
                    c_alpha_max_neighbors = args['c_alpha_max_neighbors'],
                    pre_transform = NormalizeProtein(filename = args['normal_file']),
                    graph_name = args['graph_name'],
                    processed_name=args['processed_name'],
                    set_length = args['set_length'],
                    # replace_graph=args['replace_graph'],
                    # replace_process=args['replace_process'])                   
                    replace_graph=True,
