...
from utils import normalize, safe_index, one_hot_res, log, dihedral, NormalizeProtein, parse_arguments
...
class PdbbindEvaluate(Dataset):
    ...
    def generate_protein_graph_evaluation(self):
        ...
        for idx, name in enumerate(tqdm(names)):        
            ...
            if score_info:
                ...
                rec_graph.score_info = score_info
            else:
                continue
    ...
    def generate_score_matrix(self, protein_dir, protein_length):
        ...
        if len(df.columns) == 3:
            df.columns = ['Unnamed: 0', 'mutant', 'score']
        ...
        result = ['' for i in range(len(df.index))]

        count = 0
        for i in df.index:
            mutant = df.loc[i,'mutant'].split(';')
            mutant_matrix = torch.zeros((len(mutant), 3))
            for j in range(len(mutant)):
                mutant_matrix[j,0] = self.amino_acids_type.index(mutant[j][0])
                mutant_matrix[j,1] = int(mutant[j][1:-1])
                mutant_matrix[j,2] = self.amino_acids_type.index(mutant[j][-1])
            mutant_score = df.loc[i, 'score']
            result[count] = [mutant_matrix, mutant_score]
            count += 1
        return result
    ...
    def get(self, idx):
        ...
        generate_graph = self.generate_one(data, idx_protein)
        # if self.use_micro:
    ...
    def generate_one(self,graph,idx_protein=0):
        # print('one')
        graph2 = Data.clone(graph)
...
if __name__ == '__main__':  
    ...
    for i in range(len(train_dataset)):
        train_dataset[i]
    print(train_dataset[10].score_info)
