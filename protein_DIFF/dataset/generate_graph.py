import os
from cath_imem_2nd import Cath_imem,dataset_argument
from torch.optim import Adam
from torch_geometric.data import Batch,Data
from utils import NormalizeProtein
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pdb_dir', type=str, default='dataset/Ago/pdb/')
    parser.add_argument('--save_dir', type=str, default='dataset/Ago/process/')
    ...
    filename_list = os.listdir(args.pdb_dir)
    for filename in tqdm(filename_list):
        if os.path.exists(args.save_dir+filename.replace('.pdb','.pt')):
            continue
        graph = pdb2graph(CATH_test_inmem, args.pdb_dir+filename)
        if graph:
            torch.save(graph, args.save_dir + filename.replace('.pdb','.pt'))
        else:
